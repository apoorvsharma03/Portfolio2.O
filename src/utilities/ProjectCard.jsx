...
const ProjectCard = ({ props, onClick, isFocused }) => {
  ...
  return (
    <div 
      id="PROJECT CARD" 
      onClick={onClick} 
      className={`flex flex-col justify-center items-center p-3 cursor-pointer rounded-2xl ${isTouchable ? 'bg-opacity-20 bg-cyan-900 shadow-2xl min-w-fit' : 'bg-opacity-20 bg-cyan-900 shadow-xl hover:bg-opacity-40 hover:shadow-2xl transition-all md:flex-row'} ${isFocused && 'bg-opacity-40 shadow-2xl md:px-4 min-w-80'}`}
    >
